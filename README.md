...
当前版本已经提供**规范排版**以及首批**来源文本清洗**能力；更复杂的来源清洗和字符转换仍是后续扩展方向：
...
- **字符转换**：随 `simplified-trad-conversion` feature 提供互斥的简繁转换（T2S/S2T，基于 OpenCC 风格词典，只改写可编辑区间、保护链接/代码/公式）；更完整的全角/半角处理与有限 Unicode 等价字符仍在规划中。
...
规则由后端注册表统一管理，当前内置 17 条（文本清洗规则和「专有名词使用正确的大小写」「不要使用不地道的缩写」「链接之间增加空格」「简体中文使用直角引号」「统一等价 Unicode 单位字符」默认关闭）。每条规则有稳定的机器 key（如 `spacing.cjk-latin`），展示名仅用于界面：
