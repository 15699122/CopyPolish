...
- **字符转换**：GUI 设置支持互斥的简繁转换（T2S/S2T，基于 OpenCC 风格词典，只改写可编辑区间、保护链接/代码/公式）；实际转换依赖 `simplified-trad-conversion` feature，默认构建中选择该项不会改变输出；更完整的全角/半角处理与有限 Unicode 等价字符仍在规划中。
