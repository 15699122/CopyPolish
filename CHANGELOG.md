...
- TUI 新增替换与字符转换请求设置面板（`Ctrl+E`）：支持有序替换项新增/编辑/启停/删除、`from`/`to` 字段切换和 `none`/`t2s`/`s2t` 模式循环；与 GUI 共用 `FormatRequest` 和 `rules.yaml`，默认构建将不可用简繁模式归一化为 `none`，feature 构建保留真实转换。预设编辑控件仍未实现。
