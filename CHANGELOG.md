...
- 复核 E2E 依赖审计：当前仍有 1 个 moderate、15 个 high 告警，涉及多个 WebdriverIO/浏览器工具传递依赖，其中包括 `serialize-javascript`；当前 npm 提供的修复路径需要 `@wdio/mocha-framework` major 升级，本轮不改动测试依赖，继续在 WebdriverIO 大版本升级时统一处理。
