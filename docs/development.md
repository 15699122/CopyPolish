...
当前工作环境可以完成 Linux/WSL 的前端、Rust、E2E typecheck 和业务语义验证，但不能替代 Windows 原生桌面验收。PR #24（`fa2fc1c`）合并到 release 前，必须在 Windows WebView2 环境重新确认设置页的主题三项间距、窄窗口 Footer、`rules.yaml` 完整路径悬停/复制、简繁转换标签间距、规则 hover 示例、键盘焦点/Space/Enter 行为，以及当前 binary 的设置保存/恢复。详细矩阵、命令、artifact 和清理要求见 [windows-e2e-runbook.md](windows-e2e-runbook.md) §1.2.1 和 [testing.md](testing.md) §7。
