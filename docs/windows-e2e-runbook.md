...
### 1.2.1 PR #24 当前 Windows 待办（2026-09-04）

PR #24（`fix/settings-dialog-polish`，被测代码提交 `fa2fc1c`）修改了设置窗口的布局、路径交互和规则展示。2026-09-03 以前的 Windows 证据来自旧 binary，不能覆盖这些新增 UI 行为；以下项目必须在包含 PR #24 的 Windows binary 上重新执行：

| 待办 | 必须确认的内容 | 完成证据 |
| --- | --- | --- |
| 主题布局 | “跟随系统 / 浅色 / 深色”三个选项等宽，选项间距统一；浅色、深色主题下文字、禁用态和焦点环可读 | 浅色/深色截图、page source 和环境 metadata |
| 窄窗口 Footer | 正常窗口及至少一个窄窗口下，设置文件区域不挤压操作按钮，底部不溢出 | `settings-narrow` artifact，人工检查通过 |
| 设置路径交互 | 页面仅显示 `rules.yaml`；悬停/聚焦可获取完整路径；鼠标点击、键盘 Enter/Space 可复制完整路径；成功/失败反馈正常 | page source + Windows 剪贴板人工确认 |
| 简繁转换布局 | “简繁转换”标签与选择框有明确垂直间距，默认构建的 T2S/S2T 禁用状态保持正确 | 设置截图和 page source |
| 规则示例提示 | 悬停规则卡片显示 `before → after` 示例；键盘聚焦 Checkbox 时可通过辅助描述获取同一示例 | page source + 鼠标/键盘交互记录 |
| 当前 binary 设置回归 | 规则、主题、字体、缩放、替换和转换设置保存/恢复，真实 Rust IPC 输出正常 | `selection-and-persistence.spec.ts` 3/3；必要时补跑 feature spec |

本章节的待办不能由 WSL、普通浏览器或 Linux GUI 替代。它们可以用于前端测试、Rust 测试和业务语义 smoke，但不能作为 Windows WebView2、Windows DPI、系统剪贴板或原生窗口布局的通过证据。
...
当前有一项历史失败回归和一组 PR #24 新增 UI 待办，原始失败 artifact 保留在 `e2e/artifacts/embedded/`：

- `selection-and-persistence.spec.ts`：旧 binary 的第三个“真实 GUI 保存替换项和简繁转换设置” case 在设置已写入后，输入 `TODO` 未得到 `待办`（本轮 artifact `1788353402936`，早先复现 artifact `1788351053668`）。当前代码已在 Linux/WSL 定向验证通过，Windows 必须重新构建并复跑确认；该历史问题不等同于当前 restart spec 的 capability 修复已通过。
- `simplified-trad-conversion.spec.ts`：首次运行因实际使用了默认 binary 而未产生转换结果；先执行 `build:app:simplified-trad` 后，s2t/t2s 均通过（2/2）。
- PR #24 设置页待办：主题三项等宽间距、`rules.yaml` 路径仅显示文件名并支持完整路径复制、简繁转换间距、规则 hover 示例和键盘辅助描述，均未被旧 GUI artifact 覆盖，必须使用 `fa2fc1c` 或其后包含 PR #24 的 Windows binary 重新留证。
