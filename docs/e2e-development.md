...
5. 读取 `[data-testid="settings-path"]`，确认可见文本为 `rules.yaml`，并通过 `title`/`aria-label` 确认目标是临时目录中的完整 `rules.yaml` 路径；
6. 关闭应用并确认进程退出；
