...
非阻断告警：E2E 依赖审计报告 16 个已知漏洞（1 个中危、15 个高危），涉及多个 WebdriverIO/浏览器工具传递依赖，其中包括 `serialize-javascript`；当前 npm 提供的修复路径要求 WebdriverIO/@wdio major 变更。Cargo 在 Windows native environment调试构建中曾报告增量缓存目录 `os error 5`，但相关编译、测试和 release 构建均以退出码 0 完成。
