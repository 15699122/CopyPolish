...
截至 2026-09-01，E2E 依赖审计报告 14 个 high，涉及多个 WebdriverIO/浏览器工具传递依赖；`serialize-javascript` 已通过 `e2e/package.json` 的 npm `overrides` 固定到 `7.1.1`，不再出现在审计结果中。剩余告警继续在依赖升级时跟踪。Cargo 在 E 盘调试构建中曾报告增量缓存目录 `os error 5`，但相关编译、测试和 release 构建均以退出码 0 完成。
