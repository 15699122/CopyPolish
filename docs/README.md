...
`v0.5.0` 发布计划已迁入 `archive/release-plans/` 保存历史上下文；GitHub 正式 Release 已于 2026-08-28 发布并标记为 latest，包含完整 6 个文件。后续开发按路线图执行。历史 Python 实现已从当前工作树移除；如需追溯，请通过 Git 历史查看，不参与当前构建、测试或行为定义。
