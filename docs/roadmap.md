...
- 增加编辑计划与旧 placeholder 路径的逐例 diff 对照；✅ 已完成并扩展至**全部稳定黄金 fixture**（`tests.rs::edit_plan_path_matches_placeholder_pipeline_on_stable_fixtures`）：语义边界类能力（文本边界、selection 门控、温标/‰、`$…$` 边界）双路径输出完全一致；剩余 35 例差异对应四类尚未迁移的生产能力（非边界规则、全角标点清理、强调/上标单位扩展边界、结构保护还原），按案例冻结在测试内 `PENDING_DIFFS` 清单，须随迁移逐项消除；
