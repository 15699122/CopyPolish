...
1. 先完成 `v0.5.0` Draft Release 的 Release Notes 复核和 latest 发布，再处理正式发布后的持续开发；
2. 每项改动先补测试与文档，再改实现；
...
- [ ] 正式 Release 资产、Release Notes 与 latest 标记复核；当前 Draft 已上传完整 6 个文件（5 个发布资产和 `SHA256SUMS`），尚未发布，latest 仍为 `v0.4.0`；
