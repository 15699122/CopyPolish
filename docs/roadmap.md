...
6. **S2-F 发布完整性**：生成生产 CycloneDX SBOM（`scripts/generate_sbom.py`，已接入 `verify.py --profile audit` 与 release workflow 的 assemble/发布），审查 Release 资产内容并确保 checksum/license 清单可重复生成；provenance/attestation 与签名机制留待后续评估接入。
