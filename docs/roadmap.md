...
- 增加编辑计划与旧 placeholder 路径的逐例 diff 对照；✅ 已完成：对照测试位于 `tests.rs::edit_plan_path_matches_placeholder_pipeline_on_semantic_fixtures`，覆盖 `measurements.yaml` 与 `mathematical-symbols.yaml` 全部案例，含「管线不改则编辑计划也不改」的单向无误报不变量。✅ 文本边界（Han↔Latin / Han↔Digit，grapheme 判定、span 内部跳过）已纳入编辑计划（`edit_plan.rs::plan_text_boundary_edits`），差异从 15 例降至 5 例；剩余差异为 selection 门控、温标/‰ 专门规则与 `$…$` 结构保护三类，冻结在测试内 `PENDING_DIFFS` 清单，须随迁移逐项消除；
