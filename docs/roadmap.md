...
- 增加编辑计划与旧 placeholder 路径的逐例 diff 对照；✅ 已完成：对照测试位于 `tests.rs::edit_plan_path_matches_placeholder_pipeline_on_semantic_fixtures`，覆盖 `measurements.yaml` 与 `mathematical-symbols.yaml` 全部案例，含「管线不改则编辑计划也不改」的单向无误报不变量。✅ 文本边界（Han↔Latin / Han↔Digit，grapheme 判定、span 内部跳过）、selection 门控（`TextBoundaryGate`）、温标/‰ 符号边缘处理与 `$…$` 结构保护均已纳入编辑计划，**全部语义 fixture 案例双路径输出一致，`PENDING_DIFFS` 清单为空（常态）**；
