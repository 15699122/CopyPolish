...
## 美元数学候选前向扫描（dev，2026-08-30）

`scan_dollar_math_spans` 原先在每个 `$` 候选处回扫前方连续反斜杠，判断奇偶转义。
现在使用前向候选扫描，在同一次遍历中维护连续反斜杠计数，并复用该逻辑寻找开、闭
定界符；单美元的换行限制、双美元匹配和未闭合行为保持不变。

新增奇偶反斜杠及单美元跨行边界测试。当前 profiling 中 `dollar_math` 约为 0.60 ms，
相较既有 0.28–0.33 ms 观测仍处于单轮运行时抖动范围内，因此不宣称稳定的平均耗时收益；
该优化的目标是降低连续反斜杠/美元候选密集输入下的重复回扫最坏情况成本。

## URL 与邮箱候选扫描合并（dev，2026-08-30）

`scan_url_email_spans` 原先分别遍历全文寻找 URL 和邮箱。现在使用一次合并正则遍历，
再根据匹配是否以 `http://` 或 `https://` 开头分类为 URL，否则分类为邮箱；URL 分支
继续保持大小写不敏感，邮箱分支保持原有字符范围与边界语义。

新增 URL、邮箱、相邻匹配和 URL 优先边界回归测试。当前结构扫描 profiling 中
`url_email` 约为 3.21 ms，相较此前约 4.90 ms 有局部改善；由于端到端 profiling
受单轮运行时抖动影响较大，不宣称稳定的总体耗时收益。

## 表格分隔线扫描去分配（dev，2026-08-30）

`scan_table_separator_spans` 原先为每个包含 `|` 的行收集临时单元格 `Vec`，随后再
统计非空单元格并验证横线内容。现在改为流式 `fold`，在单次遍历中完成计数和合法性
判断，保持首尾空单元格过滤、可选首尾冒号、至少三个横线字符以及至少两个单元格的
语义。
