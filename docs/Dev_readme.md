...
13. `registry.rs` 的 `RulePhase` 是规则调度的显式第一层：pipeline 通过 `execution_rules()` 按阶段稳定排序，同阶段继续保持注册表顺序；`rules()` 仍用于稳定的 UI 展示顺序。后续阶段再补充 before/after 依赖和循环检测。
 14. Unicode 等价识别与输出规范化分离：识别层把 `µ/μ`、`Å/Å` 视为等价语义，输出层默认不改写用户原文；如提供统一表示，须作为独立、默认关闭的规范化规则并评估 NFKC 影响。
 15. 复杂输入测试应至少覆盖结构保护、语义 token、普通文本规则三层同时命中的场景；新增规则或保护层改动必须补充复杂组合 fixture、规则选择组合、优先级和幂等性测试。当前 `structure-precedence.yaml` 作为 pending 基线记录行内代码与单位/数学扫描的优先级冲突，不能把该差异误标为稳定行为。
