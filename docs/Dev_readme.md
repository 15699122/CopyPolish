...
3. 占位符格式为 `\u{E000}CCWPROTECTED{n}\u{E001}`；保护范围包括文档开头 YAML front matter、表格分隔行、HTML block、引用式链接定义、fenced code block、HTML 注释、LaTeX 环境/display/inline/command、支持嵌套括号目标的 Markdown 图片/链接/autolink、任意长度同 delimiter 的行内代码、URL、邮箱、缩进代码行及化学式。
