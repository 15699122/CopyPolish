...
- **未完成**：Span/Edit 尚未接管 `format_text`；现有生产路径仍是 placeholder + 逐行 `fn(&str) -> String` 规则；`structure-precedence.yaml` 仍是 pending 基线。编辑计划与旧 placeholder 路径的逐例 diff 对照已建立：文本边界、selection 门控、温标/‰ 与 `$…$` 边界均已纳入编辑计划，全部语义 fixture 双路径输出一致（`PENDING_DIFFS` 为空）；生产接管（R3 收尾）待后续推进。
